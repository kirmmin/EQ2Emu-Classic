...
int32 Transmute::CreateItemRequest(Client* client, Player* player) {
	...
	for (auto& itr : *il) {
		...
		if (ItemIsTransmutable(item) && MinimumSkillCheck(item, client, player, false)) {
			transmutables.push_back(item->details.unique_id);
		}
	}
...
bool Transmute::MinimumSkillCheck(Item* item, Client* client, Player* player, bool b_send_error) {
	...
	bool pass = (skill && skill->current_val >= required_skill);

	if (!pass && b_send_error) {
...
void Transmute::HandleItemResponse(Client* client, Player* player, int32 req_id, int32 item_id) {
	...
	if (!MinimumSkillCheck(item, client, player, true)) {
		return;
	}
