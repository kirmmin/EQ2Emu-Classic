				...
				if(packet){
					...
					if (sep && sep->arg[0][0]) {
						if (strcmp(sep->arg[0], "wall") == 0) {
							...
							placement_mode = 1;
						else if (strcmp(sep->arg[0], "openheading") == 0 && target->IsWidget()) {
						...
						else if (strcmp(sep->arg[0], "closeheading") == 0 && target->IsWidget()) {
		...
		case COMMAND_LOOT_LIST: {
			...
			if (spawn && spawn->IsEntity()) {
				if (sep && sep->arg[0]) {
					...
					if (!strcasecmp(sep->arg[0], "remove") && sep->arg[1] && sep->IsNumber(1))
					{
					...
					else if (!strcasecmp(sep->arg[0], "add") && sep->arg[1] && sep->IsNumber(1))
					{
					...
					else if (!strcasecmp(sep->arg[0], "clearall"))
					{
		...
		case COMMAND_CASTSPELL: {
			if (sep && sep->arg[0] && sep->IsNumber(0))
			{
				...
				if (spell)
				{
					...
					spellProcess->CastInstant(spell, (Entity*)client->GetPlayer(), (target && target->IsEntity()) ? (Entity*)target : (Entity*)client->GetPlayer());
				}
				else
				{
