...
bool LoginServer::Process() {
	...
	while((pack = tcpc->PopPacket()))
	{
		...
		case ServerOP_CharacterCreate:{
			...
			DumpPacket(pack->pBuffer + sizeof(int16),pack->size - sizeof(int16));
			PacketStruct* packet = configReader.getStruct("CreateCharacter", version);
			int8 resp = 0;
			int32 acct_id = 0;
			int32 char_id = 0;
			memcpy(&acct_id, pack->pBuffer + sizeof(int16), sizeof(int32));
			if(packet && packet->LoadPacketData(pack->pBuffer + sizeof(int16) + sizeof(int32), pack->size - sizeof(int16) - sizeof(int32))){
				packet->PrintPacket();
