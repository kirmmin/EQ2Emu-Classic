...
void Client::SendCharInfo(){
	...
	/*LogWrite(CCLIENT__DEBUG, 0, "Client", "Setting Equipped Item Appearances...");
	player->SetEquippedItemAppearances();*/

	LogWrite(CCLIENT__DEBUG, 0, "Client", "Sending Character Data...");
	ClientPacketFunctions::SendCharacterData ( this );

	LogWrite(CCLIENT__DEBUG, 0, "Client", "Sending CharPOVGhost...");
	SendCharPOVGhost();

	PacketStruct* packet = configReader.getStruct("WS_SetControlGhost",GetVersion());
	if(packet){
		LogWrite(CCLIENT__DEBUG, 0, "Client", "Sending WS_SetControlGhost.");
	...
	//LogWrite(CCLIENT__DEBUG, 0, "Client", "Sending Character Sheet...");
	//ClientPacketFunctions::SendCharacterSheet(this);

	//LogWrite(CCLIENT__DEBUG, 0, "Client", "Sending Trait List...");
...
bool Client::HandlePacket(EQApplicationPacket *app) {
	...
	//}
		LogWrite(PACKET__DEBUG, 0, "Received %s Opcode", app->GetOpcodeName());
		DumpPacket(app, true);
