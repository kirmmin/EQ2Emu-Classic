...
int32 LoginDatabase::GetServerByNameCharIdAccount(string* name, int32 char_id, int32 account_id) {
	Query query;
	MYSQL_ROW row;
	MYSQL_RES* result = query.RunQuery2(Q_SELECT, "SELECT server_id from login_characters where name='%s' and char_id=%lu and account_id=%lu and deleted = 0", getSafeEscapeString(name).c_str(), char_id, account_id);
...
LoginAccount* LoginDatabase::LoadAccount(const char* name, const char* password, bool attemptAccountCreation){
	...
	if(result){
		...
		{
			Query newquery;
			newquery.RunQuery2(Q_INSERT, "insert into account set name='%s',passwd=md5('%s')", query.escaped_name, query.escaped_pass);
