...
int32 PackClassic(const char* input, sint32 inputSize, int8* outBuf, sint32 outBufSize) {
	assert(outBufSize > 4);
	sint32 inputCount = inputSize;
	int8* outBufStart = outBuf;
	outBufSize -= 4;
	outBuf += 4;
	memset(outBuf, 0, outBufSize);

	while (inputCount) {
		sint32 code = min<>(7, inputCount);
		sint32 zeroLen = 0;
		zeroLen = 0;
		int8 bVar5 = 0;

		auto ZeroFillBlock = [&code, &zeroLen, &input, &inputCount, &outBuf] {
			cout << "ZeroFillBlock() executing" << endl;
			inputCount -= code;
			...
			*outBuf = zeroLen;
			++outBuf;
			cout << "--> ZeroFillBlock() ending" << endl;
		};

		if (code == 0) {
			cout << "Execute code == 0 ZeroFillBlock()" << endl;
			ZeroFillBlock();
		}
		else {
			bool bRedo;
			do {
				...
				*outBuf = bVar5 | 0x80;
				...
				zeroLen = 0;
				do {
					if ((static_cast<int32>(bVar5 | 0x80) & 1 << (zeroLen & 0x1f)) != 0) {
	...
	int32 packSize = outBuf - outBufStart;
	memcpy(&outBufStart, &packSize, sizeof(int32));
